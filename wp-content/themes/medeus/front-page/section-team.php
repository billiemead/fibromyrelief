<div class="front_page_section front_page_section_team<?php
	$medeus_scheme = medeus_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $medeus_scheme ) && ! medeus_is_inherit( $medeus_scheme ) ) {
		echo ' scheme_' . esc_attr( $medeus_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( medeus_get_theme_option( 'front_page_team_paddings' ) );
	if ( medeus_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$medeus_css      = '';
		$medeus_bg_image = medeus_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$medeus_anchor_icon = medeus_get_theme_option( 'front_page_team_anchor_icon' );
	$medeus_anchor_text = medeus_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( medeus_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$medeus_css      = '';
			$medeus_bg_mask  = medeus_get_theme_option( 'front_page_team_bg_mask' );
			$medeus_bg_color_type = medeus_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $medeus_bg_color_type ) {
				$medeus_bg_color = medeus_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$medeus_caption = medeus_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $medeus_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $medeus_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $medeus_caption, 'medeus_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$medeus_description = medeus_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $medeus_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $medeus_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $medeus_description ), 'medeus_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! medeus_exists_trx_addons() ) {
						...
						medeus_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
