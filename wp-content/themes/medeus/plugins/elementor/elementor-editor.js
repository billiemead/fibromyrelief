...
jQuery( document ).ready(
	function() {
		...
		medeus_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="medeus_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.medeus_post_editor' );
						if ( link.length > 0 ) {
							medeus_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.medeus_post_editor', function(e) {
						medeus_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('medeus_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function medeus_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
