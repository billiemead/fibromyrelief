...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			medeus_create_observer( 'medeus-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				medeus_gutenberg_editor_init();
			} );
		} else {
		 	medeus_gutenberg_editor_init();
		}
	...
	function medeus_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'medeus_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'medeus_inited' ) ) {
	...
	// Init on page load
	function medeus_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = medeus_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + MEDEUS_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + MEDEUS_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in MEDEUS_STORAGE['override_classes'] ) {
				$obj.addClass( MEDEUS_STORAGE['override_classes'][i].replace( '%s', MEDEUS_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( medeus_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && MEDEUS_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'medeus_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			medeus_remove_observer( 'medeus-check-editor-styles-wrapper' );
			medeus_create_observer( 'medeus-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'medeus_inited' );
	}
} );
