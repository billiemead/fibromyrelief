...
if ( function_exists( 'medeus_elementor_is_preview' ) && medeus_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			medeus_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	medeus_new_main_query(
		array(
			'post_type'      => medeus_get_theme_option( 'post_type' ),
			'category'       => medeus_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'medeus_filter_get_template_part', medeus_blog_archive_get_template() ) );
}
