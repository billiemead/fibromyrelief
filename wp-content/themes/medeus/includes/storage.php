...
if ( ! function_exists( 'medeus_storage_get' ) ) {
	/**
	 ...
	 */
	function medeus_storage_get( $var_name, $default = '' ) {
		global $MEDEUS_STORAGE;
		return isset( $MEDEUS_STORAGE[ $var_name ] ) ? $MEDEUS_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'medeus_storage_set' ) ) {
	/**
	 ...
	 */
	function medeus_storage_set( $var_name, $value ) {
		global $MEDEUS_STORAGE;
		$MEDEUS_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'medeus_storage_empty' ) ) {
	/**
	 ...
	 */
	function medeus_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $MEDEUS_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $MEDEUS_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $MEDEUS_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'medeus_storage_isset' ) ) {
	/**
	 ...
	 */
	function medeus_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $MEDEUS_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $MEDEUS_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $MEDEUS_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'medeus_storage_unset' ) ) {
	/**
	 ...
	 */
	function medeus_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $MEDEUS_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $MEDEUS_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $MEDEUS_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'medeus_storage_inc' ) ) {
	/**
	 ...
	 */
	function medeus_storage_inc( $var_name, $value = 1 ) {
		global $MEDEUS_STORAGE;
		if ( empty( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = 0;
		}
		$MEDEUS_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'medeus_storage_concat' ) ) {
	/**
	 ...
	 */
	function medeus_storage_concat( $var_name, $value ) {
		global $MEDEUS_STORAGE;
		if ( empty( $MEDEUS_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'medeus_storage_get_array' ) ) {
	/**
	 ...
	 */
	function medeus_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $MEDEUS_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $MEDEUS_STORAGE[ $var_name ][ $key ] ) ? $MEDEUS_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'medeus_storage_set_array' ) ) {
	/**
	 ...
	 */
	function medeus_storage_set_array( $var_name, $key, $value ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$MEDEUS_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'medeus_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function medeus_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ][ $key ] ) ) {
			$MEDEUS_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$MEDEUS_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$MEDEUS_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'medeus_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function medeus_storage_merge_array( $var_name, $key, $value ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$MEDEUS_STORAGE[ $var_name ] = array_merge( $MEDEUS_STORAGE[ $var_name ], $value );
		} else {
			$MEDEUS_STORAGE[ $var_name ][ $key ] = array_merge( $MEDEUS_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'medeus_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function medeus_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			medeus_array_insert_after( $MEDEUS_STORAGE[ $var_name ], $after, $key );
		} else {
			medeus_array_insert_after( $MEDEUS_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'medeus_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function medeus_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			medeus_array_insert_before( $MEDEUS_STORAGE[ $var_name ], $before, $key );
		} else {
			medeus_array_insert_before( $MEDEUS_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'medeus_storage_push_array' ) ) {
	/**
	 ...
	 */
	function medeus_storage_push_array( $var_name, $key, $value ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $MEDEUS_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $MEDEUS_STORAGE[ $var_name ][ $key ] ) ) {
				$MEDEUS_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $MEDEUS_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'medeus_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function medeus_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $MEDEUS_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $MEDEUS_STORAGE[ $var_name ] ) && is_array( $MEDEUS_STORAGE[ $var_name ] ) && count( $MEDEUS_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $MEDEUS_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $MEDEUS_STORAGE[ $var_name ][ $key ] ) && is_array( $MEDEUS_STORAGE[ $var_name ][ $key ] ) && count( $MEDEUS_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $MEDEUS_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'medeus_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function medeus_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( empty( $MEDEUS_STORAGE[ $var_name ][ $key ] ) ) {
			$MEDEUS_STORAGE[ $var_name ][ $key ] = 0;
		}
		$MEDEUS_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'medeus_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function medeus_storage_concat_array( $var_name, $key, $value ) {
		global $MEDEUS_STORAGE;
		if ( ! isset( $MEDEUS_STORAGE[ $var_name ] ) ) {
			$MEDEUS_STORAGE[ $var_name ] = array();
		}
		if ( empty( $MEDEUS_STORAGE[ $var_name ][ $key ] ) ) {
			$MEDEUS_STORAGE[ $var_name ][ $key ] = '';
		}
		$MEDEUS_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'medeus_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function medeus_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $MEDEUS_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $MEDEUS_STORAGE[ $var_name ] ) ? $MEDEUS_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $MEDEUS_STORAGE[ $var_name ] ) ? $MEDEUS_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'medeus_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function medeus_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $MEDEUS_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $MEDEUS_STORAGE[ $var_name ]->$prop ) ? $MEDEUS_STORAGE[ $var_name ]->$prop : $default;
	}
}
